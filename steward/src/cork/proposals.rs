...
use gravity_bridge::gravity_proto::cosmos_sdk_proto::cosmos::base::{
    query::v1beta1::PageRequest, tendermint::v1beta1::GetLatestBlockRequest,
};
use somm_proto::{
    cork::ScheduledCorkProposal, cosmos_sdk_proto::cosmos::base::abci::v1beta1::TxResponse,
};
...
pub async fn start_scheduled_cork_proposal_polling_thread() -> JoinHandle<()> {
    ...
    tokio::spawn(async move {
        ...
        loop {
            ...
            state
                .update_latest_observed_height(config.cosmos.grpc.clone())
                .await;
            ...
            if content.type_url == "/cork.v2.ScheduledCorkProposal" {
                ...
                let encoded_call: Vec<u8> = match governance_call.call.unwrap() {
                    ...
                    Call::CellarV1(data) => {
                        ...
                        let function = data.function.unwrap();
                        cellar_v1::get_encoded_governance_call(
                            function,
                            &cellar_id,
                            proposal.proposal_id,
                        )?
...
async fn retry_failed_schedulings(
    state: &mut ProposalThreadState,
    config: &StewardConfig,
    schedule_function: fn(String, Vec<u8>, u64) -> Result<TxResponse, Error>,
) {
    if state.retry_data.is_empty() {
        return;
    }
    ...
    for (i, mut d) in state.retry_data.iter_mut().enumerate() {
        if let Err(err) = schedule_function(d.1.clone(), d.2.clone(), d.3) {
            ...
            if d.4 as u64 >= config.cork.max_scheduling_retries {
                warn!(
                    "proposal {} failed to schedule, will not be retried again.",
                    d.0
                );
...
mod tests {
    ...
    fn test_retry_failed_schedulings() {
        ...
        // all fail
        let erroring_function =
            |_: String, _: Vec<u8>, _: u64| -> Result<TxResponse, crate::error::Error> {
                Err(ErrorKind::GovernanceCall.context("error").into())
            };
        block_on(retry_failed_schedulings(
            &mut state,
            &test_config,
            erroring_function,
        ));
