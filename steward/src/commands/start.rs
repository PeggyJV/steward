...
pub struct StartCmd {
    ...
    /// TO-DO: There is a bug with the Override implementation not updating the config properly.
    /// For now we will just have to explicitly set enable to `true` in the config file.
    #[clap(long)]
    pub simulate: bool,

    /// TO-DO: Same problem as above.
    #[clap(long)]
    pub tls: bool,
}
...
impl Runnable for StartCmd {
    ...
    fn run(&self) {
        ...
        info!(
            "simulate flag: {}, simulate enabled in config: {}",
            self.simulate, config.simulate.enabled
        );
