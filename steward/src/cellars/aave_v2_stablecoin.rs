...
pub fn get_encoded_call(function: Function, cellar_id: String) -> Vec<u8> {
    match function {
        EnterStrategy(_) => {
            info!("encoding aave_v2_stablecoin.enterStrategy call for cellar {}", cellar_id);
            let call = EnterStrategyCall {};
            let call = AaveV2StablecoinCellarCalls::EnterStrategy(call);
            call.encode()
        }
        ReinvestAmount(params) => {
            info!("encoding aave_v2_stablecoin.reinvestAmount call for cellar {}", cellar_id);
        ...
        Reinvest(params) => {
            info!("encoding aave_v2_stablecoin.reinvest call for cellar {}", cellar_id);
        ...
        ClaimAndUnstakeAmount(params) => {
            info!("encoding aave_v2_stablecoin.claimAndUnstakeAmount call for cellar {}", cellar_id);
        ...
        ClaimAndUnstake(_) => {
            info!("encoding aave_v2_stablecoin.claimAndUnstake call for cellar {}", cellar_id);
            let call = ClaimAndUnstakeCall {};
            AaveV2StablecoinCellarCalls::ClaimAndUnstake(call).encode()
        }
        Rebalance(params) => {
            info!("encoding aave_v2_stablecoin.rebalance call for cellar {}", cellar_id);
        ...
        AccruePlatformFees(_) => {
            info!("encoding aave_v2_stablecoin.accruePlatformFees call for cellar {}", cellar_id);
            let call = AccruePlatformFeesCall {};
            AaveV2StablecoinCellarCalls::AccruePlatformFees(call).encode()
        }
        TransferFees(_) => {
            info!("encoding aave_v2_stablecoin.transferFees call for cellar {}", cellar_id);
            let call = TransferFeesCall {};
            AaveV2StablecoinCellarCalls::TransferFees(call).encode()
        }
        SetInputToken(params) => {
            info!("encoding aave_v2_stablecoin.setInputToken call for cellar {}", cellar_id);
        ...
        RemoveLiquidityRestriction(_) => {
            info!("encoding aave_v2_stablecoin.removeLiquidityRestriction call for cellar {}", cellar_id);
            let call = RemoveLiquidityRestrictionCall {};
            AaveV2StablecoinCellarCalls::RemoveLiquidityRestriction(call).encode()
        }
        Sweep(params) => {
            info!("encoding aave_v2_stablecoin.sweep call for cellar {}", cellar_id);
            let call = SweepCall {
                token: params
                    .address
                    .parse::<H160>()
